...
def dec():
    ...
    x = layers.Dense(2622, name="dense_2")(x)
    x = layers.LeakyReLU(name="lrelu_1")(x)

    x = layers.Dense(2622, name="dense_3")(x)
    ...
    # Body
    # The sum in `sliders` is the output lengths of the commented dense layers +4+3+2+9+3+3+4+3+3+2
    ...
    continuous = tf.concat(
        [sliders, gender,ff1_intensity,ff2_intensity], axis=2)
    mix_sliders = tf.concat([skintone, headshape_preset, headshape_refine_dir,
    ...
    out = (continuous,mix_sliders)
    # out = tf.concat([sliders, gender,skintone, headshape_preset, headshape_refine_dir,
    #                      eyes_sliders,eyes_mix, eye_colour,eyebrows_sliders, eyebrows_preset,
    ...
    #                      ff1_preset, ff2_preset, ff1_intensity, ff2_intensity],axis=2)
...
def cat_cross_entropy(y_true, y_pred):
    ...
    for idx1, idx2 in slices:
        if idx2 - idx1 == idxDiff:
            ...
            loss.append(tf.reduce_sum(
                tf.losses.categorical_crossentropy(YTRUE, YPRED)))
...
def train_step(inp, gt, dec):
    with tf.GradientTape() as decoder:
        # generated = dec(inp)
        continuous, mix = dec(inp)
        print('CONTINOUS',continuous)
        tf.print(mix)
        continuous = tf.cast(continuous, tf.float64)
        loss = custom_loss(gt, continuous)
...
def train(dataset, epochs):
    ...
    for epoch in range(epochs):
        ...
        print(
            f'TRAINING LOSS: {np.mean(loss)} Time for epoch {epoch + 1} is {time.time()-start} sec', end='\r')
